...
/*   Updated: 2025/06/03 19:13:12 by tiaperei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	update_redirection(t_data *data, t_command *cmd, char *type, char *file)
{
	if (ft_strcmp(type, "<") == 0)
	{
		free(cmd->input_file);
...
static void	handle_redirection(t_data *data, t_command *cmd, t_args **current)
{
	...
	update_redirection(data, cmd, type, file);
}
