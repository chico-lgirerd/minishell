/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   executer_utils.c                                   :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: lgirerd <lgirerd@student.42lyon.fr>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/19 11:17:09 by lgirerd           #+#    #+#             */
/*   Updated: 2025/06/30 17:06:57 by lgirerd          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "parsing.h"
#include "colors.h"
#include "libft.h"
#include "cmd.h"
#include "errors.h"
#include "files.h"
#include <sys/stat.h>
#include <unistd.h>

int	handle_empty_cmd(t_data *data, t_command *cmd)
{
	int			saved_fds[2];
	t_command	*curr;

	curr = cmd;
	while (curr)
	{
		if ((curr->input_file && access(curr->input_file, F_OK) != 0))
		{
			ft_putstr_fd(RED"minishell: "RESET, 2);
			ft_putstr_fd(curr->input_file, 2);
			ft_putstr_fd(RED": No such file or directory\n"RESET, 2);
			check_for_output(curr, data, saved_fds);
			return (1);
		}
		if (curr->out_redir && curr->out_redir->filename[0] == '\0')
		{
			ft_putstr_fd(RED"minishell: : No such file or directory\n"RESET, 2);
			return (1);
		}
		setup_redirection(curr, data, saved_fds);
		curr = curr->next;
	}
	restore_fds(saved_fds, data);
	return (0);
}

void	handle_path(char *path, char *cmd, t_data *data, char **env_arr)
{
	if (!path || cmd[0] == '\0')
	{
		free_chars(env_arr);
		if (cmd[0] == '\0')
			free(path);
		exit(handle_not_found(cmd, data));
	}
	if (ft_strcmp(cmd, ".") == 0)
	{
		free(path);
		free_chars(env_arr);
		exit(handle_point(data));
	}
	else if (ft_strcmp(cmd, "..") == 0)
	{
		free(path);
		free_chars(env_arr);
		exit(handle_not_found(cmd, data));
	}
	else if (ft_strcmp(path, "NOPERM") == 0)
	{
		free_chars(env_arr);
		exit(handle_noperm(cmd, data));
	}
	handle_other_path(path, cmd, data, env_arr);
}

void	handle_other_path(char *path, char *cmd, t_data *data, char **env_arr)
{
	if (ft_strcmp(path, "NOFILE") == 0)
	{
		free_chars(env_arr);
		exit(handle_nofile(cmd, data));
	}
	else if (is_directory(path))
	{
		free_chars(env_arr);
		exit(handle_isdir(cmd, data));
	}
}

int	is_directory(char *path)
{
	struct stat	path_stat;

	if (stat(path, &path_stat) != 0)
	{
		ft_putendl_fd(RED"minishell: stat failed"RESET, 2);
		return (-1);
	}
	return (S_ISDIR(path_stat.st_mode));
}

int	handle_isdir(char *cmd, t_data *data)
{
	ft_putstr_fd(RED"minishell: "RESET, 2);
	if (cmd)
		ft_putstr_fd(cmd, 2);
	ft_putendl_fd(RED": Is a directory"RESET, 2);
	free_all_data(data, true);
	return (126);
}
